...
MAGAZA_VERI_TABANI = {
    ...
    "AVM (Fast Food & Restoran)": {
        "Burger King": {
            ...
            "🍦 Külah Dondurma": {"fiyat": 30.00, "kisaltma": "Klh Dndrm Sde", "sodyum": "Düşük", "bpa_risk": "Sıfır", "kalori": 140, "katki": "Kıvam Artırıcı"},
            "🥧 Sıcak Elmalı Turta": {"fiyat": 55.00, "kisaltma": "Sck Elml Trt", "sodyum": "Orta", "bpa_risk": "Düşük", "kalori": 290, "katki": "Aroma Arttırıcı"}
        },
        "Tavuk Dünyası": {
            ...
            "🥗 Ekstra Akdeniz Salatası": {"fiyat": 80.00, "kisaltma": "Ekst Akdnz Slt", "sodyum": "Düşük (150mg)", "bpa_risk": "Sıfır", "kalori": 190, "katki": "Yok"},
            "🥤 Cam Şişe Coca-Cola": {"fiyat": 45.00, "kisaltma": "Cm Sse Kla", "sodyum": "Orta", "bpa_risk": "Sıfır (Cam Şişe)", "kalori": 180, "katki": "Fosforik Asit"}
        }
    }
}

# Session State Başlatma (Verilerin sayfalar arası taşınması için)
if "asama" not in st.session_state: st.session_state["asama"] = "giris"
if "sepet" not in st.session_state: st.session_state["sepet"] = []

# =====================================================================
# 🧭 ÇOKLU SAYFA MİMARİSİ (YÖNLENDİRME)
# =====================================================================
...
if sayfa == "🏠 1. Giriş & Mağaza Kurulumu":
    ...
    if st.button("Setup'ı Tamamla ve Kasaya Git ➔", type="primary"):
        if m_adi and m_tel:
            ...
            st.success("✔️ Kurulum Başarılı! Şimdi sol menüden veya aşağıdaki butondan '2. Dijital Sipariş Ekranı' sayfasına geçiniz.")
        else:
    ...
    if "magaza_adi" not in st.session_state:
        ...
        if st.session_state["asama"] in ["qr_goster", "bitti"]:
            ...
            with col_qr:
                ...
                canli_site_url = f"https://efis-portal.streamlit.app/?id={st.session_state['siparis_no']}&user={st.session_state['musteri']}"
            ...
            with col_phone:
                if st.session_state["asama"] == "bitti":
                    ...
                    st.info("💡 Şimdi sol menüden '3. AI Toksisite Analiz Merkezi' sayfasına geçerek hocaya yapay zeka katmanlarını gösterin!")
...
# ---------------------------------------------------------------------
    ...
    if "siparis_no" not in st.session_state:
        ...
        with st.expander("🧠 1. DEEP LEARNING KATMANI (LSTM Modelleme)", expanded=True):
            ...
            st.info(f"**LSTM Model Sonucu:** *{magaza}* veri tabanındaki tarihsel sipariş yoğunluğu analiz edildi. Yarın bu saatlerde **{sepet[0]}** ürününe yönelik talebin **%{skor}** oranında artacağı derin öğrenme ağımızca doğrulanmıştır. Tedarik lojistiği otonom tetiklendi.")
        ...
        with st.expander("🎨 3. GEN AI (Üretken Kişiselleştirilmiş Sağlık Raporu)", expanded=True):
            st.warning(f"**GenAI Tarafından Sıfırdan Üretilen Özgün Metin (Müşteri: {st.session_state['musteri']}):**\n\nBugün tercih ettiğiniz ürünlerin kalori yükü yüksektir. Fiziksel termal fiş basımını engelleyerek elinize bulaşacak olan kanserojen BPA hormon bozucusundan tamamen korundunuz. Üretken modelimiz bu öğünden sonra 45 dakika kardiyo egzersizi ve bol antioksidan tüketmenizi önermektedir.")
